...
function insertPosition(code: string, pathname: string) {
  ...
  transform(ast, {
    nodeTransforms: [
      (node) => {
        if (node.type === 1 && node.tagType === 0 && !EXCLUDED_TAG.includes(node.tag)) {
...
function __hideVueInspectorPosition(options) {
  ...
  return options;
    ...
    // Vue 3
    s.replace(/(createElementVNode|createElementBlock) as _\1,?/g, (match, name) => {
      names.add(name);
...
async function getInspectorStyle() {
  const content = await fs.readFile(path.join(__dirname, '../inspector/style.css'), 'utf-8');
...
function generateOptions(options: OptionModule) {
...
export const unpluginFactory: UnpluginFactory<Options | undefined> = (options) => {
  ...
  if (vuePkg.version.startsWith('3.') || true) {
    plugins.push({
  ...
  plugins.push({
    ...
    load(id) {
      ...
      if (id.startsWith(`\0${VIRTUAL_ID}/style`)) {
        return getInspectorStyle();
      }

      if (id.startsWith(`\0${VIRTUAL_ID}/options`)) {
        return generateOptions({ cwd, modifierKey, dataKey: DATA_KEY });
      }
